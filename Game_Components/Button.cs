...
namespace Tetris.Game_Components
{
    ...
    public class Button : Panel
    {
        ...
        public Button(Point position, int width, int height, string button_text = "Button", string button_graphic_path = null)
            : base(position, width, height)
        {
            ...
            button_state = Button_State.Active; // Ustawienie domyślnego stanu przycisku na aktywny
        }
        ...
        public void Load_Content(SpriteBatch spriteBatch, ContentManager content)
        {
            ...
            {
                button_image = content.Load<Texture2D>(button_graphic_path);
            }

            // Ustawienie kolorów przycisku
            ...
            // Inicjalizacja cache kolorów przycisku
            Initialize_Cache();
        }
        ...
        {
            foreach (Button_State state in Enum.GetValues(typeof(Button_State)))
            {
        ...
        private void Set_Button_States()
        {
            ...
            if (is_mouse_over && button_state == Button_State.Active || is_mouse_over && button_state == Button_State.ActiveHover)  // Jeśli myszka jest nad przyciskiem
            {
                ...
                if (mouseState.LeftButton == ButtonState.Pressed && previousMouseState.LeftButton == ButtonState.Released)              // Jeśli lewy przycisk myszy jest wciśnięty nad przyciskiem
                {
                    ...
                    Sound_Manager.Sound_Button.Play();  // Odtworzenie dźwięku przycisku
                }
            }
